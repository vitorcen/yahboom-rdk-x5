...
I2C 崩溃重生间隙，7.6V 欠压区崩溃频发与 [[rdk-x5-nav2-plan]] 教训 6 印证）+**系统 Tab**
（内嵌 SVG 板卡框图：主板内 SoC/内存/WiFi + USB3 HUB 扩展板横条（雷达/手柄/底盘串口实物上
都挂 HUB 下）+ CSI 相机 + 底盘扩展板（IMU 可点击）+ OLED + ROS2 栈，模块可点击→右侧面板
ssh 拉设备文件/服务/进程/日志，状态灯 10s 轮询；硬件拓扑与软件栈合并一张 SVG——栈上下
倒置（驱动层贴外设行、UI 在底），外设连线直接穿透到对应驱动节点列，传感数据下行/控制上行
（mux→driver 唯一上行箭头），栈整体套大虚框+淡靛蓝底色，主板/栈标题白粗体可点击；
电机/IMU/电池内嵌底盘块；OLED 物理链=RDK 40pin 排针→**风扇扩展板**（亚博，顺延排针）
→OLED 排线插角落引脚，图上画在主板与 HUB 之间；亚博硬件（底盘板）+亚博软件统一橙框；
右栏单一详情面板硬件/栈节点共用（节点出身四色：
上游=Nav2 容器/madgwick/ekf/joy_node/rosbridge/oradar，tros=mipi_cam/hobot_codec/:8000 预览，
亚博=Mcnamu_driver/joy_ctrl/oled.py(栈外)，自研=cmd_vel_mux/GUI），栈节点可点击→右下第二
详情面板（进程/**程序与配置路径**——unit+ExecStart 用 u() 助手 systemctl 现查、参数 yaml 从
活进程 cmdline 现抠/话题 hz 实测/journal 摘录/rosbridge 在线客户端），底层硬件格联动右上硬件
详情。板端路径速查：亚博工作区 /home/sunrise/yahboomcar_ws/install/、自研 /home/sunrise/
nav_config/、地图 /home/sunrise/maps/、雷达驱动在 software/library_ws、tros=/opt/tros/humble、
上游=/opt/ros/humble、unit 全在 /etc/systemd/system/（仓库 board/ 镜像）。
**mux 实际只有两级**：/cmd_vel_joy(手柄,高) > /cmd_vel(Nav2 和键盘共用,低) → /cmd_vel_drv，
没有 /cmd_vel_nav 这个话题。坑：Nav2 的 amcl/planner 等是
**组合节点**同跑一个 component_container_isolated，pgrep 按节点名永远查不到，要按容器进程/
systemd-cgls 探测；探测脚本 echo 文本里含节点名也会被 pgrep -f 反咬——已统一用 pg() 助手（滤 bash -c+空则显
"未运行"）。**两个亚博 APP 的通道**（板上核实）：建图导航 APP=rosbridge :9090 客户端（订
/map+/robot_pose——robot_pose_publisher_ros2 专为 APP 把 TF 转普通话题；发 goal/initialpose/
cmd_vel；调 **/yahboomAppSaveMap** 服务（WebSaveMap.srv，板端跑 nav2 map_saver_cli 存图），
包 yahboom_app_save_map）；手机遥控 APP=TCP :6000 控制+:6500 MJPEG（Flask/gevent），经
app_SunriseRobot.py 直写串口完全绕过 ROS，与 driver 抢串口故已禁自启。**亚博全套源码在仓库
source/**（yahboomcar_ws/src+sunriseRobot+software，2026-07-11 下载），源码核实的事实：driver
发 /imu/data_raw+/imu/mag+/vel_raw+/voltage，/vel_raw→**base_node**(C++)积分成 /odom_raw→ekf；
madgwick **use_mag=false** 磁力计没参与融合；SELECT=buttons[6] 切 Joy_active 同时发零速刹停。
设备实名：雷达 /dev/oradar→ttyACM0、底盘 /dev/myserial→ttyUSB0 均 USB 转串（HUB=VIA 2109:0817
USB3+2817 USB2 双芯片）；OLED=SSD1306 128×32 @ i2c-5 0x3C，yahboom_oled 服务跑 app 目录
oled.py；九轴 IMU 无独立设备节点，经 STM32 串口→driver→/imu/data_raw(10Hz)→madgwick→/imu/data；
tros 相机走 vin/isc 无 /dev/video*），日志 Tab=/rosout 实时
