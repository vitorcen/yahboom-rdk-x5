...
> 传感器布局：顶部 ORADAR MS200 激光雷达（360° / 10 Hz）；车头 **USB Orbbec Astra Pro
> 结构光深度相机**（深度 + 彩色，替换原 IMX219 CSI 相机）；四轮麦克纳姆轮（全向移动）。
...
| USB Astra 深度相机预览（裸 OpenNI2） | ✅ 实测 | §4 |
...
| `astra-cam` | USB Astra 深度相机预览节点（§4，替换退役的 CSI `mipi-cam`） |
...
> 注：跟随基于原 IMX219 相机开发，现相机已换 USB Astra，跟随链接回 Astra 彩色（含几何适配）待办。
...
## 4. USB Astra 深度相机预览（裸 OpenNI2，实测 ✅）

车头换成 **USB Orbbec Astra Pro** 结构光深度相机，深度伪彩进 GUI 左上窗（红近蓝远）。自研节点
[`astra_preview.py`](board/home/sunrise/nav_config/astra_preview.py)，`astra-cam.service` 开机自启：

- **弃用亚博 `astra_camera` ROS 驱动**——它开机关 IR 激光、`set_ldp_enable` 又报错关不掉，导致深度恒 0；
  改用**裸 OpenNI2（primesense）直读**深度（30 fps 稳定），LUT 上色 → `/camera/depth/color_jpeg` @15 fps。
- **活性看门狗**：只发新帧（冻结即停发，GUI 掉流灯生效）+ 掉线/卡死即进程退出让 systemd 重启（兼顾热插拔）。
- **USB2 物理限制**：深度（bulk）与彩色（UVC isochronous）带宽互斥，同开彩色会把深度砸到 ~1.5 fps，
  故**彩色默认关**（`enable_color:=true` 才开）；供电吃紧，需车子供电足或独立供电 hub。
- **量程**：约 0.6–8 m，太近/太远/玻璃反光/黑色吸红外都无回波（画面对应区域黑，属正常）。

踩坑全记（激光默认关、LDP、带宽矩阵、供电、判据用 rclpy 探针）在 `.memory/rdk-x5-astra-depth.md`。
...
| 深度相机 | **Orbbec Astra Pro**（USB，结构光深度 + 彩色 UVC；走 OpenNI2）。原 Sony IMX219 CSI 已退役 |
...
│   │                               #   （bringup/mux/joy_teleop/episode_*/dog_walk/astra_preview/map_save）
