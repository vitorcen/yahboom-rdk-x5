...
1. **客户端模式（当前默认）**：板子作为 WiFi 客户端接入家里路由器 `YOUR_WIFI_SSID_5G`。
   - 前提：**你的电脑必须连在同一个路由器/局域网**（`192.168.13.x` 网段），否则 `192.168.13.187` 根本 ping 不通。
   - 连接：`ssh root@192.168.13.187`（密码 `yahboom`）。
...
ping -b 192.168.13.255 -c 3 2>/dev/null; ip neigh | grep -i '18:ce:df:79:2e:8b'
...
> 路由器绑定了**静态 IP `192.168.13.187`**。日常用 `ssh root@192.168.13.187`（密码 `yahboom`）。
> 下面的 `192.168.8.88` 是 **AP 热点模式**下的地址，跑 `wifi_ap.sh` 切回热点时才用。

| 项目                             | 值                                               |
| -------------------------------- | ------------------------------------------------ |
| **当前 IP（client 模式）** | `192.168.13.187`（路由器静态绑定，5G）          |
...
```bash
# 当前（client 模式）
ssh root@192.168.13.187         # 密码 yahboom
```
...
| 服务          | 地址                          | 说明                                          |
| ------------- | ----------------------------- | --------------------------------------------- |
| JupyterLab    | `http://192.168.13.187:8888` | 官方交互式教程 / 例程                         |
| VNC (x11vnc)  | `192.168.13.187:5900`        | 远程桌面，**密码 `123456`**（已配置） |
| 相机 Web 预览 | `http://192.168.13.187:8000` | tros websocket（需先接好相机，见 §6）        |
...
| 项     | 值                                                                            |
| ------ | ----------------------------------------------------------------------------- |
| 地址   | `192.168.13.187:5900`（display `:0`）                                      |
| 密码   | `123456`                                                                    |
...
```bash
ssh root@192.168.13.187                                   # 先登录板子
sudo /home/sunrise/scripts/camera_mode.sh tros      # 切到 tros 预览模式
# 然后你电脑浏览器打开：  http://192.168.13.187:8000
```
...
- tros `/image_raw` 实测约 **30 FPS**，浏览器预览监听 `http://192.168.13.187:8000`。
